...
class Room {
  ...
  TiltedRec *_rectangles;

public:

  Room(int nb_cameras, int nb_positions);
  ~Room();

  inline int nb_positions() const { return _nb_positions; }
  inline int nb_cameras() const { return _nb_cameras; }
  inline int view_width(int n_camera) const { return _view_width[n_camera]; }
  inline int view_height(int n_camera) const { return _view_height[n_camera]; }
  inline int set_camera_size(int n_camera, int h, int w){
    _view_height[n_camera] = h;
    _view_width[n_camera] = w;
  }

  inline TiltedRec *avatar(int n_camera, int n_position) const {
