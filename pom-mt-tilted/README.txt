...
------------------
  ...
  ROOM <number of cameras> <number of locations>

    Defines the the number of cameras and the number
    of locations in the area of interest.

  CAMERA <camera id> <view width> <view height> 
    Defines the image size for camera <camera id>

  TILTEDREC <camera number> <location number> notvisible| <xplot> <yplot> n {<xmin> <ymin> <xmax> <ymax>}

    Defines the parameters of a certain tilted rectangle, standing for an
    individual at a certain location viewed from a certain camera. All
    non-specified rectangles are "not visible" by default.
    <xplot> and <yplot> are only used for plotting dots, followed by a set of n rectangles descriptions.
