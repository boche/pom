...
POMSolver::POMSolver(Room *room) {
    neg = new ProbaView*[room->nb_cameras()];
    neg_view = new ProbaView*[room->nb_cameras()];
    ii_neg = new IntegralProbaView*[room->nb_cameras()];
    ii_neg_view = new IntegralProbaView*[room->nb_cameras()];
...
void POMSolver::compute_average_images(int camera,
  ...
  for(int n = 0; n < room->nb_positions(); n++) if((*proba_absence)[n] <= global_proba_ignored) {
    TiltedRec *r = room->avatar(camera, n);
    if(r->visible)
      for(int i = 0; i < r->stripes.size(); i++)
        neg[camera]->multiply_subarray(r->stripes[i].xmin, r->stripes[i].ymin, r->stripes[i].xmax + 1,
                              r->stripes[i].ymax + 1, (*proba_absence)[n]);
...
void POMSolver::add_log_ratio(int camera,
  ...
  double sv = ii_neg_view[camera]->compute_sum(neg[camera], view);

  scalar_t average_surface = room->view_width(camera) * room->view_height(camera) - s;
  ...
  for(int i = 0; i < room->nb_positions(); i++) {
    TiltedRec *r = room->avatar(camera, i);
    if(r->visible) {
      scalar_t lambda = 1 - 1/(*proba_absence)[i];

      scalar_t integral_neg = 0;
      for(int irec = 0; irec < r->stripes.size(); irec++)
        integral_neg += ii_neg[camera]->integral(r->stripes[irec].xmin, r->stripes[irec].ymin,
                                        r->stripes[irec].xmax + 1, r->stripes[irec].ymax + 1);
      ...
      scalar_t integral_neg_view = 0;
      for(int irec = 0; irec < r->stripes.size(); irec++)
        integral_neg_view += ii_neg_view[camera]->integral(r->stripes[irec].xmin, r->stripes[irec].ymin,
                                        r->stripes[irec].xmax + 1, r->stripes[irec].ymax + 1);
//      ii_neg_view.integral(r->xmin, r->ymin, r->xmax + 1, r->ymax + 1);
