...
void Room::save_stochastic_view(char *name,
  ...
  for(int px = 0; px < _view_width[n_camera]; px++)
    for(int py = 0; py < _view_height[n_camera]; py++)
  ...
  for(int n = 0; n < nb_positions(); n++) {
    ...
    if(r->visible) {
      ...
      if(r->xmin > 0 && r->xmax < _view_width[n_camera]-1 && r->ymax < _view_height[n_camera]-1)
